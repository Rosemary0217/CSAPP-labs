...
void eval(char *cmdline) 
{
    ...
    if ((pid = fork()) == 0){   //子进程中解除屏蔽
        sigprocmask(SIG_UNBLOCK, &prev_child, NULL);
        setpgid(0,0);   //子进程变为一个独立的线程组，去执行命令
...
void sigchld_handler(int sig) 
{
    ...
    while((pid=waitpid(-1, &status, WNOHANG|WUNTRACED))>0){
        sigprocmask(SIG_BLOCK, &mask, &prev_mask); //防止出现该进程不是被父进程杀死，jobs中没有删除，前台程序一直等待导致timeouterror的情况
...
void sigint_handler(int sig) 
{
    ...
    sigset_t mask, prev_mask;
    sigfillset(&mask);
    sigprocmask(SIG_BLOCK, &mask, &prev_mask);  //访问jobs之前阻塞所有信号
    pid_t target_pid = fgpid(jobs);    //获取前台作业的pid
